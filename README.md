...
该项目是使用 `YOLOv5 v5.x` 的程序来训练在智能工地安全领域中头盔目标检测的应用;

## 支持 YOLOv5 - 5.x（2021.9 更新！）
目前已支持 YOLOv5 的 5.x 版本
如果使用 `v2.x` 的版本，请用 `V3.1` 的 tag 
...
运行命令：
```shell script
python detect.py --source ./area_dangerous --weights ./weights/helmet_head_person_s.pt --dangerous_area
```
